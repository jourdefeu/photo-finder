...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "help_how_it_works":
        instructions = (
            ...
            "2. Я найду похожие снимки\n\n"
            "3. Скачайте все сразу"
        )
...
def main():
    ...
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE" or not BOT_TOKEN:
        ...
        logger.info("Установите токен в переменной BOT_TOKEN")
        logger.info("=" * 60 + "\n")
        return
    ...
    if not os.path.exists(TEMP_DIR):
        ...
        logger.info(f"📁 Создана папка для временного хранения фото от пользователя: {TEMP_DIR}")
