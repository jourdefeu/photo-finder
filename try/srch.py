...
import faiss
import numpy as np
from detect import FaceDetector      # класс-детект

def find_face(filename):                                 #  img_334.jpg
    detector = FaceDetector(device="cpu")

    vector_dir = "try/vector_db"                         # путь к FAISS базе
    users_dir = "try/photos/users"                       # путь к фоткам
    temporary_dir = "try/photos/temporary"               # путь к временному хранилищу фоток, отправляемых пользователями
    ...
    if input_path.lower().endswith((".jpg", ".jpeg", ".png", ".webp")):
        # -- детекция лица
        success = detector.detect_and_draw(input_path)
...
if __name__ == "__main__":
    ...
    returned_data = find_face(filename)
    print(f"Найденные фото: {returned_data["user_photos"]}")
